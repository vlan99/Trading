...
int start()
{
    if(StringFind(Symbol(),"CORN",0)<0) return(0);
...
//----
    ...
    for(int i=0; i<OrdersTotal(); i++) 
    {
        ...
        if (StringFind(OrderSymbol(),"CORN",0)>=0)
        {
    ...
    {
       ObjectSet("MaxLotSize",OBJPROP_COLOR,Gold);
       Text_MaxLotSize=StringConcatenate(DoubleToStr(MaxRiskLots-AvailableLots,2)," / ",DoubleToString(MaxRiskLots,2));
    }
    else
    {  
       ObjectSet("MaxLotSize",OBJPROP_COLOR,Lavender);
       Text_MaxLotSize=StringConcatenate("Too risky! (",DoubleToStr(AvailableLots,2)," / ",DoubleToString(MaxRiskLots,2),")");
    }
    if(TotalOpenOrderLots!=0 && TotalOrderRiskLots==0) 
    {
        ...
        Text_MaxLotSize=StringConcatenate(DoubleToStr(AvailableLots,2)," / ",DoubleToString(MaxRiskLots,2));
    }
